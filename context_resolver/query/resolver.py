...
demanded by a :meth:`~context_resolver.context.context.Context.query` call.
It never eagerly traverses the entire tree.

The resolver depends **only** on the
:class:`~context_resolver.inference.strategy.ResolutionStrategy` interface
(via :class:`~context_resolver.query.passes.ResolutionPass`).
...
3. Otherwise:
    ...
    a. Run all registered :class:`~context_resolver.query.passes.DeterministicPass` objects.
   b. If the node is now ``FULLY_SPECIFIED``, cache and return it.
    c. If the node is a :class:`~context_resolver.ast.resolvable_node.ResolvableNode`:

      * Resolve all dependency paths (recursively, via query).
      * Build the resolution request from the template and bindings.
    * Call the :class:`~context_resolver.inference.strategy.ResolutionStrategy`.
...
from context_resolver.ast.nodes import Node, NodeState, MappingNode, SequenceNode, _node_from_dict
...
from context_resolver.ast.schema import SchemaValidationError
...
class Resolver:
    ...
    :class:`~context_resolver.inference.strategy.ResolutionStrategy` interface
    (via :class:`~context_resolver.query.passes.ResolutionPass`) and never
    imports any concrete provider or strategy implementation.

    Parameters
    ----------
    template_registry:
        Registry of named :class:`~context_resolver.templates.template.Template` objects.
    passes:
        Ordered list of :class:`~context_resolver.query.passes.QueryPass` objects
...
def _extract_scalar(node: Node) -> Any:
    ...
    For a :class:`~context_resolver.ast.nodes.ScalarNode` this is just
    :attr:`~context_resolver.ast.nodes.ScalarNode.value`.
    For a resolved :class:`~context_resolver.ast.resolvable_node.ResolvableNode`
    this recursively extracts the result.
    For compound nodes a dict/list is returned.
    """
    from context_resolver.ast.nodes import ScalarNode, MappingNode, SequenceNode
...
def _decode_response(data: dict[str, Any]) -> Node:
    ...
    Scalar values become :class:`~context_resolver.ast.nodes.ScalarNode` instances.
    Nested dicts become :class:`~context_resolver.ast.nodes.MappingNode` instances.
    Lists become :class:`~context_resolver.ast.nodes.SequenceNode` instances.
    """
    from context_resolver.ast.nodes import ScalarNode, MappingNode, SequenceNode
