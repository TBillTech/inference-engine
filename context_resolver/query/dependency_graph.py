"""
Dependency graph for the query engine.

The :class:`DependencyGraph` tracks which :class:`~context_resolver.ast.resolvable_node.ResolvableNode`
...
class DependencyGraph:
    """
    Directed dependency graph over :class:`~context_resolver.ast.paths.Path` nodes.
    ...
    * what depends on a given path (its *dependents*)
    ...
    def add_dependency(self, dependent: Path, dependency: Path) -> None:
        ...
        ----------
        dependent:
            The path of the node that needs to be resolved.
        dependency:
            The path of the node whose value is consumed during resolution
            of *dependent*.
