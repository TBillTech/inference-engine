...
:class:`~context_resolver.ast.resolvable_node.ResolvableNode` should be resolved –
the semantic procedure.  It is separate from the
:class:`~context_resolver.inference.provider.ResolutionProvider`, which
...
* :class:`PromptStrategy` – the default strategy; renders a prompt and
    delegates to a single :class:`~context_resolver.inference.provider.ResolutionProvider`.
"""
...
if TYPE_CHECKING:
    from context_resolver.inference.provider import ResolutionProvider, ResolutionRequest, ResolutionResult
...
class ResolutionStrategy:
    ...
    :class:`~context_resolver.ast.resolvable_node.ResolvableNode` – it is concerned
    ...
    * ``CompositeStrategy`` – combines multiple strategies.
    ...
    a single :class:`~context_resolver.inference.provider.ResolutionProvider`.
    ...
    def provider(self) -> "ResolutionProvider | None":
        """
        The primary :class:`~context_resolver.inference.provider.ResolutionProvider`
...
class PromptStrategy(ResolutionStrategy):
    ...
    :class:`~context_resolver.inference.provider.ResolutionProvider`.

    This is the strategy that corresponds to the existing behaviour – it passes
    the :class:`~context_resolver.inference.provider.ResolutionRequest` directly
    to the wrapped provider's :meth:`~context_resolver.inference.provider.ResolutionProvider.resolve`
    ...
    ----------
    provider:
        The :class:`~context_resolver.inference.provider.ResolutionProvider`
