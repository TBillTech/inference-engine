...
from context_resolver.ast.nodes import Node, MappingNode, SequenceNode, ScalarNode
...
def diff_contexts(old: Node, new: Node) -> list[ContextDiff]:
    ...
    :class:`~context_resolver.ast.nodes.ScalarNode` values are compared for
...
def _summarise(node: Node | None) -> Any:
    ...
    if node is None:
        return None
    from context_resolver.ast.nodes import ScalarNode
