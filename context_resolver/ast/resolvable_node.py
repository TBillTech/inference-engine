...
:class:`~context_resolver.inference.provider.ResolutionProvider` implementations.
...
class ResolvableNode(Node):
    ...
    ----------
    template_ref:
        A string identifier for the :class:`~context_resolver.templates.template.Template`
        that drives this node's resolution step.
    input_bindings:
        A mapping from template variable names to :class:`~context_resolver.ast.paths.Path`
    ...
    def state(self) -> NodeState:
        """
        Map :class:`ResolvableNodeState` to :class:`~context_resolver.ast.nodes.NodeState`.
...
from context_resolver.ast.nodes import register_node_type
